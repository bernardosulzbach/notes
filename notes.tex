...
This document is a collection of notes, formulas, and solved problems regarding
various subjects I need to remember. It exists mainly for reference purposes.

\textbf{I strongly suggest you not to print this document}.
...
\section{Exercises}
Exercises should be presented in the \texttt{Exercise} environment with two
paragraphs, which should be started by \textbf{Problem.} and
\textbf{Solution.}, respectively. An example of such configuration is presented
here.

\begin{Exercise*}
    \paragraph{Problem.} The problem.
    \paragraph{Solution.} The solution.
\end{Exercise*}
...
\begin{ExerciseList}
\Exercise
Suppose that the probability of an item on an assembly line being defective is \(p = 0.02\).
What is the average number of items inspected before finding the first defective item?
\Answer
\[\frac{1}{p} = \frac{1}{0.02} = 50\]
\Exercise What is the probability that none of the first ten sampled items are not defective?
\Answer
...
\begin{Exercise}
\paragraph{Question.} Find the slope of the surface \(f(x, y) = x^{3}y^{2} + 5y\) in the y-direction at the point \((1, -2)\).
\paragraph{Answer.} First, we differentiate with respect to \(y\) to find an expression for the slope we are interested in.
